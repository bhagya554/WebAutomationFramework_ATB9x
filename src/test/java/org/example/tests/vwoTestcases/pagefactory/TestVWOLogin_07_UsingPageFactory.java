package org.example.tests.vwoTestcases.pagefactory;
...
public class TestVWOLogin_07_UsingPageFactory extends BaseTest {
    ...
    @Description("Verify that with valid email, pass, dashboard is displayed")
    @Test
    public void test_positive_vwo_loginFailed() throws Exception {
        logger.info("Started the execution of testcase | test_positive_vwo_login failed ");
        LoginPage loginPage = new LoginPage(driver);
        loginPage.loginToVWOWithValidCredentials(PropertiesReader.readKey("vwo_username"),PropertiesReader.readKey("vwo_password"));
        ...
        assertThat(usernameLoggedIn).isNotEmpty().isNotBlank().isNotNull();
        Assert.assertEquals(usernameLoggedIn,"bhagya");
